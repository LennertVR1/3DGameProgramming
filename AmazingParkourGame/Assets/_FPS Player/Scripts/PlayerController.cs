...
public enum Status { idle, walking, crouching, sprinting, sliding, climbingLadder, wallRunning, vaulting, grabbedLedge, climbingLedge, surfaceSwimming, underwaterSwimming, scalingLedge }
...
public class PlayerController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        switch (status)
        {
            ...
            case Status.vaulting:
                VaultMovement();
                break;
            case Status.scalingLedge:
                ShuffleLedgeMovement();
                break;
            default:
                DefaultMovement();
                break;
    ...
    {
        //Vector3 input = playerInput.input;
        //Vector3 move = Vector3.Cross(Vector3.right, wallNormal).normalized;
        //move *= input.x;
        //move.x = input.x * movement.walkSpeed;
    ...
    void ClimbLedgeMovement()
    {
        ...
        playerInput.ResetJump();
        movement.Move(move, movement.crouchSpeed, 0f);
    ...
    void ShuffleLedgeMovement()
    {
        pushFrom = transform.position;
        if (shuffleDirection)
        {
            movement.Move(Vector3.back, movement.walkSpeed, 0f);
        }
        else
        {
            movement.Move(Vector3.forward, movement.walkSpeed, 0f);
        }
        if (playerInput.input.magnitude < 0.02f)
    ...
    void UpdateLedgeGrabbing()
    {
        ...
        if (status != Status.climbingLedge)
        {
            ...
            if (status == Status.grabbedLedge)
            {
                ...
                else if (playerInput.down.x >= 1)
                {
                    //movement.Move(Vector3.right, 50f, 0f);
                    //movement.Move((Vector3.right + transform.right).normalized, 1f, 0f);
                    canGrabLedge = true;
                    shuffleDirection = true;
                    ChangeStatus(Status.scalingLedge);
                }
                else if (playerInput.down.x <= -1)
                {
                    //movement.Move(Vector3.left, 50f, 0f);
                    //movement.Move((Vector3.left - transform.right).normalized, 1f, 0f);
                    canGrabLedge = true;
                    shuffleDirection = false;
                    ChangeStatus(Status.scalingLedge);
                }
            }
        }
